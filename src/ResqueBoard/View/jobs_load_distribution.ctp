	            ...
	            if ($cacheDriver->contains($cacheId)) {
					...
					foreach ($jobsMatrix as $matrix) :
						...
						if ($matrix['value'] > 0) {
							...
							$date->format('Y-m-d') . ' ' . str_pad($currentHour, 2, '0', STR_PAD_LEFT) . ':59:59" data-html="true" data-event="tooltip" title="<b>' . number_format($matrix['value']) . ' jobs</b>on<br/>' . $date->format('l d M Y \a\t H\h') . '">';
						}
